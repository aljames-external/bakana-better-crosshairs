...
export class BaseCrosshairShape {
    ...
    async onPlacedCallback(crosshair, ...extraArgs) {
        ...
        if (Sequencer?.EffectManager?.endEffects) {
            try {
                ...
                log.debug("BaseCrosshairShape.onPlacedCallback | Exception ending Sequencer effects:", e);
            }
        }
    ...
    rotate(newAngleDeg, refresh = true) {
        ...
        if (this.placeable) {
            try { this.placeable.direction = newAngleDeg; } catch (e) {}
        ...
        if (refresh) {
            this.refreshTemplateHighlights();
            try { this.sequencerCrosshair?._refreshShape?.(); } catch (e) {}
            try { this.sequencerCrosshair?._refreshTemplate?.(); } catch (e) {}
            try {
                this.sequencerCrosshair?.renderFlags?.set?.({
                    refreshShape: true,
                    refreshTemplate: true,
                    refreshState: true
                });
                this.sequencerCrosshair?.applyRenderFlags?.();
            } catch (e) {}
