...
export class AutorecMenuApplication extends BaseCrosshairMenuApplication {
    ...
    protected override async _prepareContext(options: any): Promise<any> {
        ...
        const entries = rawEntries.map((e: any) => ({
            ...
            rectangleFile: e.rectangleFile ?? e.squareFile ?? DEFAULT_AUTOREC_ENTRY.rectangleFile,
        ...
        const labels = {
            ...
            showItemIcon: localize("BBC.autorecMenu.labels.showItemIcon", "Show Item Icon"),
            showItemIconLabel: localize("BBC.autorecMenu.labels.showItemIconLabel", "Display item icon on Sequencer crosshairs"),
    ...
    /**
     * Subclass hook executed during ApplicationV2 rendering lifecycle.
     * Binds application-specific interactive event handlers.
     * @protected
    ...
    _attachCustomEventListeners(root, context, options) {
        ...
        const editToggle = rootEl.querySelector("#bbc-edit-mode-toggle");
        ...
        if (addWorkflowBtn) {
            addWorkflowBtn.addEventListener("click", async () => {
                ...
                try {
                    ...
                    log.debug("AutorecMenuApplication | Add workflow dialog cancelled or closed:", e);
                    result = null;
                }
        ...
        const removeSelectedBtn = rootEl.querySelector(".bbc-remove-selected-btn");
        if (removeSelectedBtn) {
            removeSelectedBtn.addEventListener("click", async () => {
                const checked = rootEl.querySelectorAll(".bbc-item-select-checkbox:checked");
                if (checked.length === 0) {
        ...
        rootEl.querySelectorAll(".bbc-save-single-btn").forEach(btn => {
            btn.addEventListener("click", async (ev) => {
                ...
                if (itemName) {
                    await this.saveSingleConfiguration(rootEl, itemName);
                }
            });
        });
        ...
        // Search Filter
        const searchInput = rootEl.querySelector("#bbc-autorec-search");
        ...
        // Restore previously selected item across re-renders
        if (this._selectedItemName) {
            this.selectItem(rootEl, this._selectedItemName);
        }
    }

    /**
     * Highlights the selected item in the sidebar and displays its configuration panel in the inspector view.
     * Entry boundary normalized for polymorphic target input.
     * @param {HTMLElement|object} target - Root HTML element or application instance.
    ...
    async saveSingleConfiguration(target, regKey) {
        ...
        for (const k of autorecManager.list()) {
            ...
            if (entry && typeof entry === "object" && !entry.local && typeof entry !== "function") {
                persistedDict[k] = entry;
            }
        }
        await autorecManager.overwrite(persistedDict);
